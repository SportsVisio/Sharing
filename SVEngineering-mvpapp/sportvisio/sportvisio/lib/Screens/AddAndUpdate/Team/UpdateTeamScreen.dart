...
import 'package:sportvisio/Widgets/CustomDropDown.dart';
...
class UpdateTeamScreen extends StatefulWidget {
  ...
  const UpdateTeamScreen({Key? key, required this.leaguesResponse});
...
class _UpdateTeamScreenState extends State<UpdateTeamScreen> {
  ...
  List<String> deletedPlayers = [];
  @override
  void initState() {
    // TODO: implement initState
    super.initState();

    selectedLeague = widget.leaguesResponse.league;
    widget.leaguesResponse.teams.map((e) => print(e.name)).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<LeaguesViewModel>(builder: (context, viewModel, child) {
      ...
      int index = _viewModel.leaguesResponses
          .indexWhere((element) => element.league == selectedLeague);
      LeaguesResponse response = _viewModel.leaguesResponses[index];
      if (selectedTeam == '') {
        selectedTeam = response.teams[0].name;
        int selectedTeamIndex = response.teams
            .indexWhere((element) => element.name == selectedTeam);
        print(selectedTeamIndex.toString() + ">>>>>");

        if (selectedTeamIndex == 0) {
          response.teams[selectedTeamIndex].players.map((e) {
            _teamsListNumbers.add(TeamNameAndController(
                controller: TextEditingController(text: e.number), id: Uuid()));
            _teamsList.add(TeamNameAndController(
                controller: TextEditingController(text: e.name), id: Uuid()));
          }).toList();
        }
      }

      //  response.teams.map((e) => _teamsListNumbers.add(TeamNameAndController(controller: e.))).toList();
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'To add/update players enter Player’s jersey numbers and player name below',
                        style: TextStyle(
                          fontFamily: 'regu',
                          fontSize: 16.0,
                      ...
                      customSearchDropdown(
                        ...
                        (var selectedData) {
                          setState(() {
                            ...
                            int index = _viewModel.leaguesResponses.indexWhere(
                                (element) => element.league == selectedLeague);
                            response = _viewModel.leaguesResponses[index];
                      ...
                      customSearchDropdown(
                        response.teams.map((e) => e.name).toList(),
                        (var selectedData) {
                          _teamsList.clear();
                          _teamsListNumbers.clear();
                          print(_teamsList.length);
                          setState(() {
                            //selectedLeague = selectedData!;
                            selectedTeam = selectedData!;
                            selectedTeam1 = selectedData;

                            int selectedTeamIndex = response.teams.indexWhere(
                                (element) => element.name == selectedTeam);
                            ...
                            response.teams[selectedTeamIndex].players.map((e) {
                              ...
                              _teamsListNumbers.add(TeamNameAndController(
                                  controller:
                                      TextEditingController(text: e.number),
                                  id: Uuid()));
                              _teamsList.add(TeamNameAndController(
                                  controller:
                                  ...
                                  Container(
                                    ...
                                    child: Center(
                                      child: SportsVisioFormField(
                                        ...
                                        controller:
                                            _teamsListNumbers[index].controller,
                                      ),
                                    ),
                                  ),
                                  Expanded(
                                    child: SportsVisioFormField(
                                      hint: "Player name ${index + 1}",
                      ...
                      MaterialButton(
                        ...
                        onPressed: () {
                          // _navigationService
                          // .navigateTo(AppRoutes.UPDATE_TEAM_CONFIRMATION);
                          if (_formKey.currentState!.validate()) {
                            var oldLeague = widget.leaguesResponse.league;
                            var newLeague = selectedLeague;
                            var oldTeam = widget.leaguesResponse.teams[0].name;
                            var newTeam = selectedTeam;
                            List<String> oldRosters = [];
                            List<String> newRosters = [];
                            List<String> RostersNumbers = [];

                            int selectedTeamIndex = response.teams.indexWhere(
                                (element) => element.name == selectedTeam);
                            print(selectedTeamIndex.toString() + ">>>>>");
                            response.teams[selectedTeamIndex].players.map((e) {
                              oldRosters.add(e.name);
                            }).toList();

                            newRosters = _teamsList
                                .map((e) => e.controller.text)
                                .toList();

                            RostersNumbers = _teamsListNumbers
                                .map((e) => e.controller.text)
                                .toList();
                            TeamUpdateResponse model = new TeamUpdateResponse(
                                newLeague: newLeague,
                                newTeam: newTeam,
                                newlayers: newRosters,
                                oldLeague: oldLeague,
                                oldPlayers: oldRosters,
                                oldTeam: oldTeam,
                                playersNumbers: RostersNumbers,
                                deletedPlayers: deletedPlayers);
                            print(model.newlayers.toString());
  ...
  void _handleDeleteTeam(int index) {
    setState(() {
      deletedPlayers.add(_teamsList[index].controller.text);
  ...
  void _handleDeleteTeamViaLeague() async {
    final Map<String, Object> rousterPayload = {};
    if (_formKey.currentState!.validate()) {
      String teamName = selectedTeam1 != '' ? selectedTeam1 : selectedTeam;
      ...
      _navigationService.navigateTo(AppRoutes.DELETE_TEAM_CONFIRMATION);
    }
  }
}
