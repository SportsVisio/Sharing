...
class AddLeagueScreen extends StatefulWidget {
  const AddLeagueScreen({Key? key}) : super(key: key);

  @override
  _AddLeagueScreenState createState() => _AddLeagueScreenState();
}
...
class _AddLeagueScreenState extends State<AddLeagueScreen> {
  ...
  Widget teamsSection() {
    return Column(
      ...
      children: [
        Row(
          children: [
            Text('Add Teams', style: heading3Style),
            Spacer(),
            Padding(
  ...
  void _handleAddLeague() async {
    final Map<String, Object> teamsPayload = {};
    if (_formKey.currentState!.validate()) {
      String leagueName = _leagueNameController.text;
      _teamsList.forEach((team) {
        teamsPayload[team.controller.text] = {};
      });
      final payload = {"League": leagueName, "Teams": teamsPayload};
      final _viewModel = Provider.of<LeaguesViewModel>(context, listen: false);
      _viewModel.setPayload(payload);
      _navigationService.navigateTo(AppRoutes.ADD_LEAGUE_CONFIRMATION);
    }
  }
